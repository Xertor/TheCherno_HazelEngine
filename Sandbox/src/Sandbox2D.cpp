...
void Sandbox2D::OnUpdate(Hazel::Timestep ts)
{
	...
	#if 0
	{
		...
		Hazel::Renderer2D::DrawQuad({ 0.0f, 0.0f, -0.1f }, { 20.0f, 20.0f }, m_CheckerboardTexture, 10.0f, glm::vec4(1.0f, 0.9f, 0.9f, 1.0f));
		Hazel::Renderer2D::DrawRotatedQuad({ -2.0f, 0.0f, 0.0f }, { 1.0f, 1.0f }, glm::radians(rotation), m_CheckerboardTexture, 20.0f);
		Hazel::Renderer2D::EndScene();

		Hazel::Renderer2D::BeginScene(m_CameraController.GetCamera());
		for(float y = -5.0f; y < 5.0; y += 0.5f)
		{
			for(float x = -5.0f; x < 5.0; x += 0.5f)
			{
...
void Sandbox2D::OnImGuiRender()
{
	...
	ImGui::Text("Vertices: %d", stats.GetTotalVertextCount());
